...
void KeyboardCallback(GLFWwindow*, int key, int scancode, int action, int modifier); //changed bcs i cant change main code at this point
...
struct GLState
{
    ...
    // Camera mode: 0..3 (3 will be FPS)
    uint32_t cameraMode = 0;

    // FPS/orbit angles (degrees, because you call glm::radians on them)
    float yaw = 90.0f;
    float pitch = 0.0f;

    // Orbit camera distance
    float orbitDist = 4.0f;


    // Time control (L faster, K slower, and K can go to reverse)
    int   timeIndex = 4;       // start at +1 (index 4 in the list below)
    float simTime   = 0.0f;
